...
(function (root, factory) {
	...
	var RxpLightbox = (function() {
		...
		function init() {
			...
			function createForm(doc){
				...
				for (var key in token) {
					if (token[key] != null) {
						...
						hiddenField.setAttribute("value", atob(token[key]));
			...
			function openWindow() {
				...
				tabWindowInterval = setInterval(function () {
					if (tabWindow.closed) {
						clearInterval(tabWindowInterval);
						closeHandler();
					}
				}, 500);
			}
	...
	var init = function(idOfLightboxButton, merchantUrl, serverSdkJson) {
		...
		if (document.getElementById(idOfLightboxButton).addEventListener) {
			document.getElementById(idOfLightboxButton).addEventListener("click", instance, true);
		} else {
			document.getElementById(idOfLightboxButton).attachEvent('onclick', instance);
		}
	...
	function setHppIframeParams (serverSdkJson) {
		...
		var hppOrigin = btoa(parser.protocol + '//' + parser.host);

		var hppVersion = serverSdkJson.HPP_VERSION ? parseInt(atob(serverSdkJson.HPP_VERSION), 10) : 1;
		if (hppVersion === 1) {
			serverSdkJson.HPP_TEMPLATE_TYPE = btoa('LIGHTBOX');
			serverSdkJson.HPP_ORIGIN = hppOrigin;
		} else {
			serverSdkJson.HPP_POST_DIMENSIONS = hppOrigin;
			serverSdkJson.HPP_POST_RESPONSE = hppOrigin;
		}
		return serverSdkJson;
	}
	...
	var setup = function (serverSdkJson, responseCallback) {
		...
		if (window.addEventListener) {
			...
			cleanup = window.removeEventListener.bind(window, 'message', receiveMessage);
		} else {
			window.attachEvent('message', receiveMessage);
			cleanup = window.detachEvent.bind(window, 'message', receiveMessage);
		}

		var instance = function () {
			var onClose = responseCallback;
			lightboxInstance.lightbox(onClose);
		};
